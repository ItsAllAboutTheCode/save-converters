#!/usr/bin/env python
"""Script for converting the The Legend of Heroes - Trails into Reverie PS4 <-> PC save"""
...
class SaveConvertReverie(SaveConvertColdSteelChecksumBase):
    ...
    def create_save_patch_table(self, convert_format: ConvertFormat) -> list[ReplaceMap]:
        ...
        for i in range(0, CHARACTER_MODELS):
            ...
            start_offset = inplace_to_skip_adjustment + 0x18204 + (LOCATION_DATA_STRIDE * i)
            bytes_to_delete = 12
            ...
            start_offset = inplace_to_skip_adjustment + 0x182E8 + (LOCATION_DATA_STRIDE * i)
            bytes_to_delete = 16
            PATCH_TABLE[PS4_TO_PC_CONVERT_FORMAT].append(
                ReplaceMap(
                    replace_functor=ReplaceRangeBytes(
        ...
        start_offset = inplace_to_skip_adjustment + 0x38E84
        bytes_to_delete = 3540
        PATCH_TABLE[PS4_TO_PC_CONVERT_FORMAT].append(
            ReplaceMap(
                replace_functor=ReplaceRangeBytes(
                    source_range=Range(start=start_offset, end=start_offset + bytes_to_delete), output_bytes=b""
        ...
        start_offset = inplace_to_skip_adjustment + 0x94E78
        bytes_to_delete = 12
        PATCH_TABLE[PS4_TO_PC_CONVERT_FORMAT].append(
            ReplaceMap(
                replace_functor=ReplaceRangeBytes(
        ...
        start_offset = inplace_to_skip_adjustment + TRAILS_INTO_REVERIE_PC_SAVE_SIZE
        bytes_to_delete = 8
        PATCH_TABLE[PS4_TO_PC_CONVERT_FORMAT].append(
            ReplaceMap(
                replace_functor=ReplaceRangeBytes(
...
def add_commands(parser: argparse.ArgumentParser) -> None:
    parser.description = "Save Converter (PS4<->PC) for Trails into Reverie"
...
def main():
    parser = argparse.ArgumentParser(
        description="Tool to convert a Trails into Reverie save between PS4 <-> PC",
