...
from save_convert.trails_of.reverie.trails_into_reverie_save_converter import (
    TRAILS_INTO_REVERIE_PC_SAVE_SIZE,
    TRAILS_INTO_REVERIE_PS4_SAVE_SIZE,
    add_commands,
    start_convert,
)
from save_convert.trails_of.trails_of_cold_steel_base_converter import decompress_savedata

SCRIPT_DIR = Path(__file__).parent.resolve()


class TestConvertTrailsIntoReverieSave(TestCase):
    def setUp(self):
        self.test_filepath: Path = Path(SCRIPT_DIR) / "test_files/reverie"

    def test_ps4_to_pc(self):
        test_file = self.test_filepath / "SAVE.ps4"
        assert test_file.exists()

        expected_ps4_save_bytes = decompress_savedata(test_file.read_bytes())
        ...
        with (
            ...
            self.assertEqual(len(test_bytes), TRAILS_INTO_REVERIE_PC_SAVE_SIZE)
            ...
            self.assertTrue(start_convert(test_args))
            self.assertEqual(len(test_bytes), TRAILS_INTO_REVERIE_PS4_SAVE_SIZE)
            self.assertSequenceEqual(expected_ps4_save_bytes, test_bytes)

    def test_pc_decompress(self):
        test_file = self.test_filepath / "SAVE.pc"
        assert test_file.exists()

        expected_pc_save_bytes = test_file.read_bytes()
        ...
        def redirect_data_byte_io(file, mode, *args):
            if mode == "wb":
                return mock_output_byteio
            return BytesIO(expected_pc_save_bytes)
        ...
        with (
            ...
            add_commands(parser)
            test_args = parser.parse_args(
                ["-i", str(test_file), "-o", str(self.test_filepath / "SAVE.decompress"), "--decompress-only"]
            )
