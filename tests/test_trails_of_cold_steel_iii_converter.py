...
from save_convert.trails_of.cold_steel_iii.trails_of_cold_steel_iii_save_converter import (
    TRAILS_OF_COLD_STEEL_III_PC_SAVE_SIZE,
    TRAILS_OF_COLD_STEEL_III_PS4_SAVE_SIZE,
    add_commands,
    start_convert,
)
...
class TestConvertTrailsOfColdSteelIIISave(TestCase):
    def setUp(self):
        self.test_filepath: Path = Path(SCRIPT_DIR) / "test_files/cold_steel_iii"
    ...
    def test_ps4_to_pc(self):
        ...
        with (
            ...
            self.assertEqual(len(test_bytes), TRAILS_OF_COLD_STEEL_III_PC_SAVE_SIZE)
            ...
            self.assertTrue(start_convert(test_args))
            self.assertEqual(len(test_bytes), TRAILS_OF_COLD_STEEL_III_PS4_SAVE_SIZE)
